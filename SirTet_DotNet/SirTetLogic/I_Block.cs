...
namespace SirTetLogic
{
    ...
    public class I_Block : Block
    {
        ...
        /// <summary>
        /// Initialize a new instance of the <see cref="I_Block"/> class
        /// </summary>
        /// <param name="middleX">Index of column</param>
        /// <param name="middleY">Index of row</param>
        public I_Block(int middleX, int middleY)
        {
            middlePoint = new Point(middleX, middleY);
            restPoints = new Point[3] { new Point(middleX - 1, middleY), new Point(middleX + 1, middleY), new Point(middleX + 2, middleY)};
        }
        /// <summary>
        ...
        public override void Rotate()
        {
            switch(rotationPose)
            {
                case 0:
                    ...
                    restPoints[2] = new Point(restPoints[2].Get()[0] - 2, restPoints[2].Get()[1] + 2);
                    rotationPose = 1;
                    break;
                case 1:
                    restPoints[0] = new Point(restPoints[0].Get()[0] + 1, restPoints[0].Get()[1] + 1);
        ...
        public override bool IfBlockOutOfGridOnRotate(int gridX)
        {
            switch(rotationPose)
            {
                case 0:
                    ...
                    break;
                case 1:
                    if(restPoints[1].Get()[0] + 1 >= gridX - 1 || restPoints[1].Get()[0] - 1 <= 0 || restPoints[2].Get()[0] - 2 <= 0)
                        return true;
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    if(restPoints[1].Get()[0] - 1 <= 0 || restPoints[1].Get()[0] + 1 >= gridX - 1 || restPoints[2].Get()[0] + 2 >= gridX - 1)
                        return true;
                    break;
            }
            return false;
        ...
        public override bool IfBlockOverrideOnRotate(bool[,] HardLayer)
        {
            ...
            {
                case 0:
                    if(HardLayer[restPoints[0].Get()[0]+1, restPoints[0].Get()[1]-1] || HardLayer[restPoints[1].Get()[0]-1, restPoints[1].Get()[1]+1] || HardLayer[restPoints[2].Get()[0]-2, restPoints[2].Get()[1]+2])
                        return true;
                    break;
                case 1:
                    if(HardLayer[restPoints[0].Get()[0]+1, restPoints[0].Get()[1]+1] || HardLayer[restPoints[1].Get()[0]-1, restPoints[1].Get()[1]-1] || HardLayer[restPoints[2].Get()[0]-2, restPoints[2].Get()[1]-2])
        ...
        public override string GetBlockType() => "I_Block";
    }
}
